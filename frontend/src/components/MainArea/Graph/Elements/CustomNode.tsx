  ...
  return (
    <>
      ...
      >
        ...
        <div
          className={`order-0 flex h-5 flex-none grow-0 flex-row items-center gap-1 self-stretch p-0 ${data.isBold ? 'justify-center' : ''}`}
          style={{
            width: data.verificationStatus === "verified" ? "160px" : "162px",
          }}
        >
          <span className={`order-0 flex h-5 flex-none grow-0 items-center overflow-hidden text-ellipsis whitespace-nowrap font-inter text-sm leading-5 tracking-normal text-node-text-primary opacity-100 ${data.isBold ? 'font-semibold' : 'font-normal'}`}>
            {data.label1}
          </span>
        ...
        <div
          className={`order-1 h-4 flex-none flex-grow-0 self-stretch overflow-hidden text-ellipsis whitespace-nowrap font-inter text-xs leading-4 text-node-text-secondary ${data.isBold ? 'font-semibold text-center' : 'font-light'}`}
          style={{
            width: "162px",
          }}
        >
