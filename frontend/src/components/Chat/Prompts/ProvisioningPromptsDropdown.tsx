...
interface ProvisioningPromptsDropdownProps {
  visible: boolean
  onSelect: (query: string) => void
}

const ProvisioningPromptsDropdown: React.FC<ProvisioningPromptsDropdownProps> = ({
  visible,
  onSelect,
}) => {
  ...
  const [provisioningPrompts, setProvisioningPrompts] = useState<string[]>([])
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const res = await fetch(`${PROVISIONING_API_URL}/suggested-prompts`, {
          cache: "no-cache",
          signal,
        })
        ...
        if (Array.isArray(data)) {
          ...
          setProvisioningPrompts(prompts)
        ...
        console.warn("Failed to load provisioning prompts from API.", err)
      }
    })()
      ...
      <div className="relative inline-block" ref={dropdownRef}>
        ...
        >
          <div className="px-2 py-2">
            {provisioningPrompts.map((item, index) => (
              <div
                key={`prompt-${index}`}
...
export default ProvisioningPromptsDropdown
