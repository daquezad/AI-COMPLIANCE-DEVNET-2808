...
export const getApiUrlForPattern = (pattern?: string): string => {
  ...
  const DEFAULT_PROVISIONING_API_URL = "http://127.0.0.1:9090"
  ...
  const PROVISIONING_API_URL =
    import.meta.env.VITE_PROVISIONING_SUPERVISOR_API_URL || DEFAULT_PROVISIONING_API_URL

  if (pattern === PATTERNS.PROVISIONING) {
    return PROVISIONING_API_URL
  } else if (isGroupCommunication(pattern)) {
...
export const supportsSSE = (pattern?: string): boolean => {
  return isGroupCommunication(pattern) || pattern === PATTERNS.PROVISIONING
}

export const isProvisioningPattern = (pattern?: string): boolean => {
  return pattern === PATTERNS.PROVISIONING
}

export const getWebSocketUrlForPattern = (pattern?: string): string => {
  if (pattern === PATTERNS.PROVISIONING) {
    const apiUrl = getApiUrlForPattern(pattern)
    return apiUrl.replace(/^http/, 'ws') + '/ws/provision'
  }
  throw new Error(`Pattern ${pattern} does not support WebSocket`)
}

export const getStreamingEndpointForPattern = (pattern?: string): string => {
  if (pattern === PATTERNS.PUBLISH_SUBSCRIBE_STREAMING || pattern === PATTERNS.PROVISIONING) {
...
export const isStreamingPattern = (pattern?: string): boolean => {
  return pattern === PATTERNS.PUBLISH_SUBSCRIBE_STREAMING || pattern === PATTERNS.PROVISIONING
}
